...
test.beforeEach(async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Ausleihen anzeigen' })).toBeVisible();
});
...
test('Show checkouts', async ({ page }) => {  
    ...
    await expect(page.getByRole('cell', { name: 'Orff De temporum fine comoedia' }).first()).toBeVisible();
  });
